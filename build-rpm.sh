...
if [[ -z "${SFOS_TARGET:-}" ]]; then
    case "$PWD" in
        *_aarch64*) SFOS_TARGET="SailfishOS-5.0.0.62-aarch64" ; TDLIB_ARCH="aarch64" ;;
        *_armv7hl*) SFOS_TARGET="SailfishOS-5.0.0.62-armv7hl" ; TDLIB_ARCH="armv7hl" ;;
        *_i486*)    SFOS_TARGET="SailfishOS-5.0.0.62-i486" ; TDLIB_ARCH="i486" ;;
        *)          SFOS_TARGET="SailfishOS-5.0.0.62-aarch64" ; TDLIB_ARCH="aarch64" ;;
    esac
fi
TDLIB_ARCH="${TDLIB_ARCH:-aarch64}"
...
# ── Verifica TDLib ────────────────────────────────────────────────────────────
TDLIB_SO="tdlib/${TDLIB_ARCH}/lib/libtdjson.so"
if [[ ! -f "$TDLIB_SO" ]]; then
